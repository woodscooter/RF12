...
//# define COMPAT		// uncomment for Hitmate compatible
#include <rf12.h>
//**********************************************************************

#ifdef COMPAT
#define UNIT_ID	0x83	// red test
#define DEST_ID	0x03	// console
#define SEC_DEST_ID	0x03
#else
// #define UNIT_ID	11
#define UNIT_ID	22
// #define UNIT_ID	33
...
static uint8_t input_1, input_2;		// control bytes for pushbuttons
static uint8_t tryPending;          	// remaining number of retries
static uint8_t porta, portc;		// shadow for outputs
static uint8_t ctrl_1, ctrl_2;		// control bytes for switch inputs
...
{
#ifdef COMPAT
	send[0] = 0x0A;		// weapon type foil, on target hit
	send[1] = 2;		// uptimer
	send[2] = 2;		// event number
	send[3] = 0xAA;
	destid = DEST_ID;
#else
...
COLD_RESET:
	...
	TRISA = 0x3E;		//	Bit 0 is an out (ICSP only)
	WPUA = 0x3E;		//	Pullup on bits 1,4,5
